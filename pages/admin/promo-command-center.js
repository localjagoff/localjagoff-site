...
function getDisplayFields(pack, selectedPlatform) {
  ...
  const shared = [
    { title: "Brand Angle", value: pack.brand_angle },
  ];
...
function SavedPackCard({ item, onLoad, onDelete, onQueue }) {
  ...
  return (
    <article className="savedCard">
      ...
      <p className="savedMeta">{displayPlatform} • {item.mode || "mode"} • {niceDate(item.createdAt)}</p>
...
export default function PromoCommandCenter() {
  ...
  useEffect(() => {
    ...
    setRecentPhrases(JSON.parse(localStorage.getItem("localJagoffRecentPromoPhrases") || "[]"));
    setSavedPacks(JSON.parse(localStorage.getItem("localJagoffSavedPromoPacks") || "[]"));
    setQueue(JSON.parse(localStorage.getItem("localJagoffPromoQueue") || "[]"));
  }, []);
  ...
  return (
    <div className="promoPage">
      <Head><title>Local Jagoff Promo Command Center</title><meta name="robots" content="noindex,nofollow" /></Head>
      <main className="wrap">
        <header className="hero"><div><p className="kicker">PRIVATE ADMIN TOOL</p><h1>Promo Command Center</h1><p>Generate Local Jagoff posts, save promo packs, queue campaign ideas, and only show the platform you picked unless Full Pack is selected.</p></div><div className="heroCard"><p>No auto-posting yet.</p><strong>You approve everything before it goes public.</strong></div></header>
        <section className="statsGrid"><div className="statCard"><span>{stats.productCount}</span><p>Products loaded</p></div><div className="statCard"><span>{stats.savedCount}</span><p>Saved packs</p></div><div className="statCard"><span>{stats.queuedCount}</span><p>Queued drafts</p></div><div className="statCard"><span>{stats.freeCount}</span><p>Free saves</p></div><div className="statCard"><span>{stats.aiCount}</span><p>AI saves</p></div></section>
