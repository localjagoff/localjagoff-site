...
function getDisplayFields(pack, selectedPlatform) {
  ...
  const fieldsWithBundles = [...fields, ...bundleFields];
  ...
  return (
    <article className="savedCard">
      ...
      <p className="savedMeta">
        {PLATFORM_LABELS[displayPlatform] || displayPlatform} • {item.mode || "mode"} • {niceDate(item.createdAt)}
      </p>
...
export default function PromoCommandCenterV6() {
  ...
  const buildHolidayNotes = () => {
    ...
    return [
      ...
      campaignStyle ? `Campaign style: ${CAMPAIGN_STYLE_LABELS[campaignStyle] || campaignStyle}` : "",
      preset?.label ? `Quick preset: ${preset.label}` : "",
      ...
      <main className="wrap">
        <header className="hero">
          <div>
            ...
            <p>Generate Local Jagoff posts, holiday campaigns, tracked links, saved promo packs, and platform-specific copy without leaving the main dashboard.</p>
          </div>
        ...
        <section className="statsGrid">
          ...
          <div className="statCard"><span>{stats.readyCount}</span><p>Ready to post</p></div>
          <div className="statCard"><span>{stats.postedCount}</span><p>Posted</p></div>
        </section>

        <nav className="dashboardNav">
          {Object.entries(SECTION_LABELS).map(([value, label]) => (
            <button key={value} type="button" className={activeSection === value ? "active" : ""} onClick={() => setActiveSection(value)}>{label}</button>
          ))}
        </nav>
        ...
        {activeSection === "create" && (
          <section className="dashboardGrid">
            <div className="panel controls">
              ...
              {mode === "holiday" && (
                <>
                  <div className="field"><label>Quick Preset</label><select value={holidayPreset} onChange={(e) => applyHolidayPreset(e.target.value)}><option value="">Manual / Custom Holiday Promo</option>{HOLIDAY_PROMO_PRESETS.map((preset) => <option key={preset.value} value={preset.value}>{preset.label}</option>)}</select></div>
                  <div className="field"><label>Holiday / Sales Event</label><select value={holidayValue} onChange={(e) => { setHolidayValue(e.target.value); setHolidayPreset(""); }}>{US_HOLIDAY_PROMOS.map((holiday) => <option key={holiday.value} value={holiday.value}>{holiday.label}</option>)}</select></div>
                  <div className="field"><label>Campaign Style</label><select value={campaignStyle} onChange={(e) => { setCampaignStyle(e.target.value); setHolidayPreset(""); }}>{CAMPAIGN_STYLES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
                  ...
                  <div className="holidayNotice full"><strong>Holiday mode:</strong> Pick a preset or customize manually. If promo details are blank, no discount is invented. Holiday packs now include UTM tracked links in the CTA helper.</div>
                </>
              )}
