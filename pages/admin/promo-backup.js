...
function writeObject(key, value) {
  if (typeof window !== "undefined") window.localStorage.setItem(key, JSON.stringify(value && typeof value === "object" && !Array.isArray(value) ? value : {}));
}
...
export default function PromoBackup() {
  ...
  const restoreFromObject = (parsed) => {
    ...
    writeArray(KEYS.performance, parsed.performance || parsed.promoPerformance || []);
    writeArray(KEYS.campaignPresets, parsed.campaignPresets || parsed.presets || []);
    writeObject(KEYS.launchChecklist, parsed.launchChecklist || parsed.checklist || {});
    writeString(KEYS.launchChecklistDate, parsed.launchChecklistDate || parsed.checklistDate || todayIso());
    refresh();
