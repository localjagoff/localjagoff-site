...
export default function PromoCalendar() {
  ...
  return (
    <div className="page">
      ...
      <main className="wrap">
        <header className="hero"><div><p className="kicker">PRIVATE ADMIN TOOL</p><h1>Promo Calendar</h1><p>Calendar-style view of queued Local Jagoff promos. Move dates, change status, and copy platform bundles.</p></div><div className="links"><a href="/admin/promo-hub">Hub</a><a href="/admin/promo-generator">Generator</a><a href="/admin/promo-queue">Queue</a></div></header>
        <section className="filters"><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>{STATUSES.map((s) => <option key={s} value={s}>{s === "all" ? "All Statuses" : s}</option>)}</select><select value={platformFilter} onChange={(e) => setPlatformFilter(e.target.value)}>{PLATFORMS.map((p) => <option key={p} value={p}>{p === "all" ? "All Platforms" : PLATFORM_LABELS[p]}</option>)}</select></section>
