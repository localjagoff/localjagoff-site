...
function platformUrl(product, platform) {
  const params = new URLSearchParams({ utm_source: platform, utm_medium: "social", utm_campaign: "builder-post", utm_content: slugify(productName(product)) });
...
function hashtags(product, platform) {
...
function makeCaptions(product, platform) {
  ...
  return [
    ...
    platform === "instagram" ? `Fresh drop from Local Jagoff.\n\n${name}\n\nLocal gear. Local attitude. No boring shit.` : `Fresh drop from Local Jagoff.\n\n${name}. Local gear with a little attitude built in.`,
  ];
}

function makeScripts(product) {
  const name = productName(product);
  return [
...
function buildOptions(product, platform) {
  const tagSets = [
    hashtags(product, platform),
    hashtags(product, platform).filter((tag) => !["#Yinzer"].includes(tag)),
    ["#LocalJagoff", "#Pittsburgh", "#WesternPA", "#SmallBusiness"],
  ];
  return { hooks: HOOKS, captions: makeCaptions(product, platform), ctas: CTAS, hashtags: tagSets, overlays: OVERLAYS, scripts: makeScripts(product) };
}

function buildFinal({ product, platform, options, selected }) {
  const hook = options.hooks[selected.hook] || "";
  const caption = options.captions[selected.caption] || "";
  const cta = options.ctas[selected.cta] || "";
  const tagLine = (options.hashtags[selected.hashtags] || []).join(" ");
  const overlay = options.overlays[selected.overlay] || "";
  const script = options.scripts[selected.script] || "";
  const link = platformUrl(product, platform);

  if (platform === "youtube_shorts") {
    return [`YouTube Shorts Title:\n${productName(product)} | Local Jagoff Drop`, `Description:\n${hook}\n\n${caption}\n\n${cta}\nLink:\n${link}\n\n${tagLine}`, `Overlay Text:\n${overlay}`, `Short Video Script:\n${script}`].join("\n\n---\n\n");
  }

  return [`${platformLabel(platform)} Post:\n${hook}\n\n${caption}\n\n${cta}`, `First Comment:\n${cta} 🖤💛`, `Link:\n${link}`, `Hashtags:\n${tagLine}`, `Image Overlay Text:\n${overlay}`, `Short Video Script:\n${script}`].join("\n\n");
}

function bankEntry(product, type, platform, text, tag) {
  return { id: `bank-${Date.now()}-${Math.random().toString(16).slice(2)}`, createdAt: new Date().toISOString(), productId: String(product?.id || ""), productName: productName(product), productImage: product?.thumbnail_url || product?.image || "", productCategory: product?.category || productType(product), type, platform, text, source: "Promo Builder", tag, status: "Approved" };
}
...
export default function PromoBuilder() {
  ...
  const saveToQueueWithStatus = (status) => {
    if (!finalPost || !product) { setMessage("Build a final post first."); return; }
    const queue = readArray(QUEUE_KEY);
    const item = { id: `builder-${Date.now()}`, queueId: `queue-${Date.now()}-${Math.random().toString(16).slice(2)}`, createdAt: new Date().toISOString(), queuedAt: new Date().toISOString(), source: "Promo Builder", mode: "builder", platform, displayPlatform: platform, scheduledPlatform: platform, scheduledDate, status, product, promo: { brand_angle: "Built from selected Promo Builder parts.", builder_final: finalPost, facebook_post: platform === "facebook" ? finalPost : "", instagram_caption: platform === "instagram" ? finalPost : "", tiktok_caption: platform === "tiktok" ? finalPost : "", youtube_shorts_description: platform === "youtube_shorts" ? finalPost : "", cta: finalPost } };
    writeArray(QUEUE_KEY, [item, ...queue].slice(0, 500));
    setMessage(`Edited final post saved to Queue as ${status}.`);
  };

  const saveSelectedToBank = () => {
    if (!options || !product) { setMessage("Generate options first."); return; }
    ...
    const entries = [
      ...
      bankEntry(product, "overlay", "general", options.overlays[selected.overlay], "Builder selected overlay"),
      bankEntry(product, "note", platform, options.scripts[selected.script], "Builder selected script"),
    ].filter((entry) => entry.text);
    writeArray(BANK_KEY, [...entries, ...bank].slice(0, 800));
    setMessage(`${entries.length} selected part${entries.length === 1 ? "" : "s"} saved to Product Bank.`);
  };
