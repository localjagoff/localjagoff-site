...
function PlatformPreview({ product, platform, destination, caption, hashtags, link, overlay, firstComment, script }) {
  ...
  return <section className="preview"><div className="previewHeader"><div className="avatar">LJ</div><div><strong>Local Jagoff</strong><span>{destinationLabel(platform, destination)}</span></div></div><div className="previewText"><p>{caption}</p>{hashtags && <p className="hashes">{hashtags}</p>}</div><div className={isVideo ? "media videoMedia" : "media"}>{image ? <img src={image} alt={productName(product)} /> : <div className="noImage">Product Image</div>}{overlay && <div className="overlay">{overlay}</div>}</div><div className="linkPreview"><span>localjagoff.com</span><strong>{productName(product)}</strong><small>{link}</small></div>{firstComment && <div className="comment"><strong>First comment</strong><p>{firstComment}</p></div>}{isVideo && script && <details className="script"><summary>Video script</summary><pre>{script}</pre></details>}</section>;
}
