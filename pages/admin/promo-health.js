...
const VALID_PLATFORMS = new Set(["facebook", "instagram", "tiktok", "youtube_shorts", "full_pack"]);
...
function normalizePlatform(value) {
  return VALID_PLATFORMS.has(value) ? value : "facebook";
}
...
function normalizeQueueItem(item) {
  const platform = normalizePlatform(item.scheduledPlatform || item.displayPlatform || item.platform);
...
function normalizeSavedItem(item) {
  const platform = normalizePlatform(item.displayPlatform || item.platform);
...
export default function PromoHealth() {
  ...
  const totalProblems = queueProblems.length + savedProblems.length;
    ...
    <div className="page">
      ...
      <main className="wrap">
        <header className="hero">
          ...
          <h1>Promo Health</h1>
          <p>Check the browser-stored promo queue, saved library, and no-repeat memory. Repair old or malformed promo data before it causes weird copy/export behavior.</p>
        </header>
        ...
        <section className={`statusPanel ${totalProblems === 0 ? "good" : "warn"}`}>
          ...
          <p>{totalProblems === 0 ? "Queue and saved promo data look usable." : "Use the repair buttons below to clean up old browser-stored promo data."}</p>
        </section>
        ...
        <section className="grid">
          ...
          <article className="panel">
            <h2>Health Report</h2>
            <p className="muted">Exports a JSON report with current counts, issues, queue, saved library, and memory.</p>
