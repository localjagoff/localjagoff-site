const { STORE_ID, HIDDEN_PRODUCT_IDS, productVisible, variantUnavailableReason,
...
async function loadProduct(id, options = {}) {
  ...
  if (HIDDEN_PRODUCT_IDS.has(id)) return null;
...
async function loadCatalog(options = {}) {
  ...
  for (let page = 0; page < 100; page++) {
    ...
    if (!data.result.length || offset > data.paging.total || page === 99) {
      throw new CommerceError("Incomplete catalog response", 503);
    }
  }
  const pending = [...ids].filter(id => !HIDDEN_PRODUCT_IDS.has(id));
  const products = [];
