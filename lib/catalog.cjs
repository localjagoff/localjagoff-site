...
function curateProduct(result, expectedId) {
  ...
  return merchandiseProduct({ id: Number(product.id), name, thumbnail_url: images[0], images,
...
function metaRows(products, origin) {
  ...
  return products.flatMap(p => p.variants.map(v => {
    ...
    return { id, item_group_id: `lj_${p.id}`, title: `${p.name} - ${v.name}`,
      ...
      // Meta's documented infinity sentinel for untracked POD inventory, not physical stock.
      // An omitted quantity has persisted as zero in Shops despite availability being in stock.
      quantity_to_sell_on_facebook: v.availability === "in stock" ? 999999 : 0,
      price: `${v.price} USD`, link: `${url.origin}/product/${p.id}?variant=${v.id}`,
      image_link: new URL(images[0], url.origin).href,
      additional_image_link: images.slice(1).map(image => new URL(image, url.origin).href).join(","),
...
function feedCsv(rows) {
  const headers = ["id", "item_group_id", "title", "description", "availability", "condition", "price",
    "link", "image_link", "additional_image_link", "brand", "size", "color", "quantity_to_sell_on_facebook"];
