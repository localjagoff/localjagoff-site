...
const { standardShippingCents, shippingQuote, FREE_SHIPPING_MINIMUM_CENTS, PRINTFUL_US } = require('./shipping-policy.cjs');
...
const GOOGLE_APPROVED_PRODUCT_IDS = new Set([
  ...
  428983169,429536493,429728777,429821634,430964873,
]);
const GOOGLE_STAGED_PRODUCT_IDS = new Set([471744647,471744585,471950476,471744283,473689891]);

function googleAttributes(id, unitAmount) {
  ...
  return { ...product, age_group: "adult", identifier_exists: "no",
    ...
    [GOOGLE_SHIPPING_FIELDS[0]]: `US:Standard Shipping:${((unitAmount === undefined ? standardShippingCents([{id,quantity:1}]) : shippingQuote([{id,quantity:1}],unitAmount).amount)/100).toFixed(2)} USD:${PRINTFUL_US.handlingMin}:${PRINTFUL_US.handlingMax}:${PRINTFUL_US.transitMin}:${PRINTFUL_US.transitMax}`,
    ...
    [GOOGLE_SHIPPING_FIELDS[4]]: `US:${(FREE_SHIPPING_MINIMUM_CENTS/100).toFixed(2)} USD`,
  };
}
