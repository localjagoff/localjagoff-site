...
const adminHandoff=require('./production-admin-handoff.cjs');
const webhookSubscriptions=require('./production-webhook-subscriptions.cjs');
...
function createPreparation(env,{fetchImpl=fetch,query,refresh=catalog.refreshStep,read=catalog.readSnapshot,handoffStore}={}){
  return {async run(action,input){
    ...
    return budget.withBudget(async()=>{
      try{
        if(action==='lifecycle-subscriptions')return await webhookSubscriptions.subscriptions(env,input,{fetchImpl});
        if(action==='admin-handoff')return await adminHandoff.handoff(env,handoffStore);
