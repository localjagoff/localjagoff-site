...
const PUBLIC_PATHS = ["/", "/contact", "/terms", "/privacy", "/whats-a-jagoff", "/arcade",
  "/yinzer-invaders", "/fry-catcher", "/bridge-rage", "/pothole-patrol",
  "/parking-chair-panic", "/jagoff-jump"];
...
function googleRows(products) {
  return catalogProducts(products).flatMap(p => [...p.variants].sort((a, b) => a.id - b.id).map(v => ({
    id: itemId(p, v), item_group_id: groupId(p), item_group_title: p.name.slice(0, 150),
    title: `${p.name} - ${v.name}`.slice(0, 150), description: p.description.slice(0, 5000),
    link: variantUrl(p, v), image_link: imageUrl(p.images[0]),
    additional_image_link: p.images.slice(1, 11).map(imageUrl).join(","),
...
function googleTsv(products) {
  ...
  return [headers.join("\t"), ...googleRows(products).map(row => headers.map(k => cell(row[k])).join("\t"))]
    .join("\n") + "\n";
}
...
module.exports = { ORIGIN, INDEXNOW_KEY, productJsonLd, sitemapUrls, sitemapXml, googleRows,
