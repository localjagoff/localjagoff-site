...
const { imagesForVariant } = require('./variant-images.cjs');
const INDEXNOW_KEY = "94e61a0c38b94f47846d1d678ac640fb";
const PUBLIC_PATHS = ["/", "/724", "/stuff-nat", "/contact", "/terms", "/privacy", "/whats-a-jagoff", "/arcade",
...
function productJsonLd(product) {
  ...
  return {
    ...
    hasVariant: variants.map(v => ({
      ...
      image: imagesForVariant(p, v).map(imageUrl), url: variantUrl(p, v),
...
function googleRows(products) {
  return catalogProducts(products).flatMap(p => {
    // Names and descriptions are shared; each color offer uses its matching mockups.
    const group = groupId(p), url = productUrl(p.id), groupTitle = p.name.slice(0, 150);
    const description = p.description.slice(0, 5000);
    ...
    link: `${url}?variant=${v.id}`, image_link: imageUrl(imagesForVariant(p, v)[0]),
    additional_image_link: imagesForVariant(p, v).slice(1, 11).map(imageUrl).join(","),
...
function googleTsv(products) {
  const { googleAttributes, GOOGLE_APPROVED_PRODUCT_IDS, GOOGLE_SHIPPING_FIELDS } = require("./google-listing-policy.cjs");
  const headers = ["id", "item_group_id", "item_group_title", "title", "description", "link",
    ...
    "age_group", "gender", "identifier_exists", ...GOOGLE_SHIPPING_FIELDS];
  ...
  const rows = googleRows(products).filter(row => GOOGLE_APPROVED_PRODUCT_IDS.has(Number(row.item_group_id.slice(3)))).map(row => ({
    ...row, ...googleAttributes(row.item_group_id.slice(3), Math.round(Number(row.price.split(' ')[0]) * 100)),
  }));
