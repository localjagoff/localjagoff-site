...
const APPROVED_PRODUCT_IDS = new Set([
  473981186,473991005,473985115,473987159,473987719,473990688,
  473808622,473834484,
  473689891,
  471744647,471744585,471950476,471744283,
  430964873,429821634,429728777,429536493,475168585,428983169,
...
function variantUnavailableReason(product, variant) {
  ...
  if (!variantColorApproved(product, variant)) return "availability";
  if (variant.currency !== "USD") return "currency";
  if (amountCents(variant.retail_price) === null) return "price";
  return null;
}

function variantColorApproved(product, variant) {
  // The owner revoked Black for this design; fail closed for unapproved colors.
  return Number(product?.id) !== 473808622 || variant?.color === "White";
}
...
module.exports = { STORE_ID, HIDDEN_PRODUCT_IDS, APPROVED_PRODUCT_IDS, getDisplayProductName,
  productVisible, variantUnavailableReason, variantColorApproved, amountCents, detectCategory, variantLabel };
