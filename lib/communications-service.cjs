...
function createService({stripe,env=process.env,store=createStore(env),fetchImpl=fetch,dispatch=deliver}={}) {
  ...
  async function snapshot(reference) {
    live(env);const checkedAt=Date.now(),saved=await store.order(reference);
  ...
  async function beforeReview(reference){
    let result;
    try{result=await snapshot(reference);}catch(error){
  ...
  async function reconcile(reference) {
    ...
    const updated=await store.query(`UPDATE comm_orders SET delivery=$2,review_due_at=$3,last_checked_at=now(),
      review_manual_reason=NULL,suppress_reviews=suppress_reviews OR $4 WHERE reference=$1 AND reconcile_generation=$5 RETURNING reference`,
    ...
    if(isProcessing(state.order)) {
      ...
      messages.push({key,kind:'processing',payload:mail.processingEmail(orderMail)});
    }
    for(const shipment of state.shipments) {
      ...
      messages.push({key,kind:'shipment',payload:mail.shipmentEmail(orderMail,shipment)});
    }
    if(messages.length)await store.query(`INSERT INTO comm_outbox(key,kind,order_ref,payload,payload_hash)
      ...
      [reference,JSON.stringify(messages.map(m=>({...m,payload_hash:hash(m.payload)}))),saved.reconcile_generation]);
    // Review timing must not discard independently valid shipment notifications.
    if(missingReviewDate&&!saved.suppress_reviews)return manualReview(saved,eligibility.reason);
    ...
    return {outcome:'reconciled',complete:Boolean(eligibility.dueAt)||(state.order.status==='fulfilled'&&saved.suppress_reviews)};
  }
