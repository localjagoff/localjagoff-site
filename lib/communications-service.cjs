...
const {reviewEligibility}=require('./customer-lifecycle.cjs');
...
function createService({stripe,env=process.env,store=createStore(env),fetchImpl=fetch,dispatch=deliver}={}) {
  ...
  async function claimCreate(reference) {
    const rows=await store.query(`INSERT INTO comm_events(identity,order_ref,kind)
      SELECT $1,$2,'fulfillment_create_attempt' FROM comm_orders
      WHERE reference=$2 AND printful_id IS NULL ON CONFLICT DO NOTHING RETURNING identity`,
      [`fulfillment-create/${reference}`,reference]);
  ...
  async function reconcile(reference,claimed) {
    ...
    const batch=pending.slice(0,4).map(m=>({key:m.key,kind:m.kind,
      payload:mail.shipmentEmail(orderMail,m.shipment)}));
  ...
  return {recordPaid,recordLinked,recordFailed,claimCreate,reconcile,beforeReview};
}
