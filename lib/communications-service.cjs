...
function createService({stripe,env=process.env,store=createStore(env),fetchImpl=fetch,dispatch=deliver}={}) {
  ...
  async function recordPaid({session,event,reference}) {
    ...
    if(await store.order(reference))return;
    // Never re-send a historical receipt merely because the new database is empty.
    if(session.metadata.order_email_sent==='true')return;
  ...
  async function recordLinked({reference,order}) {if(await store.order(reference))await store.link(reference,order.id);}
  ...
  return {recordPaid,recordLinked,reconcile,beforeReview};
}
