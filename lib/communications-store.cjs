...
async function alertOwner(store,reference,reason) {
  await store.enqueue(`lifecycle-alert/${reference}`,'owner_alert',reference,{
    from:ORDER_SENDER,to:[SUPPORT],reply_to:SUPPORT,
    subject:`HIGH PRIORITY: lifecycle verification requires review ${reference}`,
    text:`Local Jagoff order ${reference}: ${reason}. Customer review messages are deferred. Inspect the existing payment and fulfillment order; do not duplicate, confirm, pay, refund or cancel without owner review.`,
  });
}
function createStore(env = process.env, {sqlClient} = {}) {
  ...
  return {
    sql, query,
    async nextFastDue() {
      const rows=await query(`SELECT min(due) AS due FROM (
        SELECT min(next_attempt_at) AS due FROM comm_outbox WHERE status='pending'
        UNION ALL SELECT min(lease_until) FROM comm_outbox WHERE status='sending'
        UNION ALL SELECT min(greatest(next_due_at,coalesce(reconcile_lease_until,next_due_at)))
          FROM comm_orders WHERE printful_id IS NOT NULL AND lifecycle_complete=false
          AND reconcile_reason='event'
      ) due_work`);
      return rows[0]?.due??null;
    },
    async order(reference,{lifecycleOnly=false}={}) {
      const columns=lifecycleOnly?'reference,session_id,printful_id,suppress_reviews,unresolved,reconcile_generation,review_manual_reason':'*';
      return (await query(`SELECT ${columns} FROM comm_orders WHERE reference=$1`,[reference]))[0];
    },
    ...
    async link(reference, id) {
      const result = await query(`UPDATE comm_orders SET printful_id=$2,unresolved=false,updated_at=now(),
        next_due_at=now(),lifecycle_complete=false,reconcile_reason='event',reconcile_generation=reconcile_generation+1
    ...
    async claimReconciliation(mode='fallback') {
      return (await query(`WITH due AS (
        SELECT reference FROM comm_orders WHERE printful_id IS NOT NULL
        AND lifecycle_complete=false AND next_due_at<=now()
        AND ($1='fallback' OR reconcile_reason='event')
        AND (reconcile_lease_until IS NULL OR reconcile_lease_until<now())
        ORDER BY next_due_at FOR UPDATE SKIP LOCKED LIMIT 1)
        UPDATE comm_orders o SET reconcile_claim=$2,reconcile_lease_until=now()+interval '5 minutes'
        FROM due WHERE o.reference=due.reference RETURNING o.reference,o.session_id,o.printful_id,
          o.customer,o.items,o.suppress_reviews,o.unresolved,o.reconcile_generation,o.reconcile_claim,
          o.review_manual_reason`,[mode,crypto.randomUUID()]))[0];
    },
    async finishReconciliation(job,{complete=false,retry=false,continuation=false}={}) {
      // A webhook arriving during the lease must survive completion of this snapshot.
      const rows=await query(`UPDATE comm_orders SET reconcile_claim=NULL,reconcile_lease_until=NULL,
        lifecycle_complete=CASE WHEN reconcile_generation=$3 THEN $4 ELSE false END,
        next_due_at=CASE WHEN reconcile_generation<>$3 THEN now() WHEN $4 THEN NULL
          ELSE now()+($5*interval '1 second') END,
        reconcile_reason=CASE WHEN reconcile_generation<>$3 OR $6 OR $7 THEN 'event' ELSE 'fallback' END,
        last_checked_at=now() WHERE reference=$1 AND reconcile_claim=$2 RETURNING reference`,
        [job.reference,job.reconcile_claim,job.reconcile_generation,complete,retry?900:continuation?15:3600,retry,continuation]);
      if(!rows.length)throw new Error('reconciliation_claim_lost');
    },
    ...
    async prepareAttempt(job,reviewCheck) {
      // One transaction fences the lease/fresh review and reserves quota before a provider attempt.
      const rows=await query(`WITH eligible AS MATERIALIZED (
        SELECT j.key FROM comm_outbox j WHERE j.key=$1 AND j.claim_token=$2
        AND j.status='sending' AND j.lease_until>now()
        AND (j.kind<>'review' OR EXISTS(SELECT 1 FROM comm_orders o WHERE o.reference=j.order_ref
          AND o.reconcile_generation=$3 AND o.suppress_reviews=false AND o.unresolved=false
          AND o.review_manual_reason IS NULL AND $4::timestamptz BETWEEN now()-interval '60 seconds' AND now()
          AND o.review_due_at<=now())) FOR UPDATE OF j),
        quota AS MATERIALIZED (SELECT comm_take_rate('mail-attempts',86400,80) AS allowed FROM eligible),
        attempted AS (UPDATE comm_outbox SET first_attempt_at=coalesce(first_attempt_at,now())
          WHERE key IN (SELECT key FROM eligible) AND (SELECT allowed FROM quota) RETURNING first_attempt_at)
        SELECT EXISTS(SELECT 1 FROM eligible) AS eligible,
          coalesce((SELECT allowed FROM quota),false) AS allowed,
          (SELECT first_attempt_at FROM attempted) AS first_attempt_at`,
        [job.key,job.claim_token,reviewCheck?.generation??null,
          Number.isFinite(reviewCheck?.checkedAt)?new Date(reviewCheck.checkedAt).toISOString():null]);
      if(!rows[0]?.eligible){
        if(job.kind==='review')return 'review_changed_before_send';
        throw new Error('outbox_claim_lost');
      }
      if(!rows[0].allowed)return 'daily_mail_budget';
      if(!rows[0].first_attempt_at)throw new Error('outbox_attempt_not_persisted');
      return 'ready';
    },
    async markAttempt(job,reviewCheck) {
      const rows=await query(`UPDATE comm_outbox SET first_attempt_at=coalesce(first_attempt_at,now())
        WHERE key=$1 AND claim_token=$2 AND status='sending' AND lease_until>now()
        AND (kind<>'review' OR EXISTS(SELECT 1 FROM comm_orders o WHERE o.reference=order_ref
          AND o.reconcile_generation=$3 AND o.suppress_reviews=false AND o.unresolved=false
          AND o.review_manual_reason IS NULL AND $4::timestamptz BETWEEN now()-interval '60 seconds' AND now()
          AND o.review_due_at<=now())) RETURNING first_attempt_at`,
        [job.key,job.claim_token,reviewCheck?.generation??null,
          Number.isFinite(reviewCheck?.checkedAt)?new Date(reviewCheck.checkedAt).toISOString():null]);
      if (!rows.length && job.kind==='review') return null;
    ...
    async cleanup() {
      await sql.transaction([
        ...
        sql.query("DELETE FROM comm_preview_events WHERE created_at<now()-interval '30 days'"),
      ],{fetchOptions:{signal:AbortSignal.timeout(15000)}});
    },
  };
}
module.exports = {createStore,hash,alertOwner};
