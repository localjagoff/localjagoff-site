  ...
  return {
    ...
    async claimReconciliation(mode='fallback') {
      return (await query(`WITH due AS (
        ...
        FROM due WHERE o.reference=due.reference RETURNING o.*`,[mode,crypto.randomUUID()]))[0];
    },
    async finishReconciliation(job,{complete=false,retry=false}={}) {
      ...
      const rows=await query(`UPDATE comm_orders SET reconcile_claim=NULL,reconcile_lease_until=NULL,
        ...
        reconcile_reason=CASE WHEN reconcile_generation<>$3 OR $6 THEN 'event' ELSE 'fallback' END,
        last_checked_at=now() WHERE reference=$1 AND reconcile_claim=$2 RETURNING reference`,
        [job.reference,job.reconcile_claim,job.reconcile_generation,complete,retry?900:3600,retry]);
