...
`ALTER TABLE comm_orders ADD COLUMN IF NOT EXISTS review_manual_reason text`,
`ALTER TABLE comm_orders ADD COLUMN IF NOT EXISTS last_checked_at timestamptz`,
`ALTER TABLE comm_orders ADD COLUMN IF NOT EXISTS next_due_at timestamptz DEFAULT now()`,
`ALTER TABLE comm_orders ADD COLUMN IF NOT EXISTS lifecycle_complete boolean NOT NULL DEFAULT false`,
`ALTER TABLE comm_orders ADD COLUMN IF NOT EXISTS reconcile_reason text NOT NULL DEFAULT 'fallback'`,
`ALTER TABLE comm_orders ADD COLUMN IF NOT EXISTS reconcile_generation bigint NOT NULL DEFAULT 0`,
`ALTER TABLE comm_orders ADD COLUMN IF NOT EXISTS reconcile_claim uuid`,
`ALTER TABLE comm_orders ADD COLUMN IF NOT EXISTS reconcile_lease_until timestamptz`,
`CREATE INDEX IF NOT EXISTS comm_orders_due ON comm_orders(next_due_at)
 WHERE lifecycle_complete=false AND printful_id IS NOT NULL`,
...
`CREATE TABLE IF NOT EXISTS comm_preview_events (identity text PRIMARY KEY, kind text NOT NULL,
 store_id bigint NOT NULL, occurred_at timestamptz NOT NULL, age_seconds integer NOT NULL,
