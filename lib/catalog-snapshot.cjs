...
const {APPROVED_PRODUCT_IDS,HIDDEN_PRODUCT_IDS,variantColorApproved}=require('./commerce-policy.cjs');
...
// Keep existing products visible while the owner-approved replacement refreshes atomically.
// Never extend the source age or expose a partially refreshed cycle.
const PREVIOUS_POLICY='[428821578,428851513,428851608,428851698,428851907,428980566,428982889,428983169,429208592,429536493,429728777,429821634,430964873,471744283,471744585,471744647,471950476,473689891,473808622,473834484,473981186,473985115,473987159,473987719,473990688,473991005]';
const RETIRED_PRODUCT_IDS=new Set([430697388,471744477,473808088,429208592]);
...
async function readSnapshot(env,{store=createSnapshotStore(env)}={}){
  ...
  const products=published.filter(p=>!RETIRED_PRODUCT_IDS.has(p?.id));
  const approved=new Set(IDS);
  if(products.some(p=>!approved.has(p?.id)))throw Error('catalog_snapshot_policy_mismatch');
  return catalogProducts(products.map(restrictSnapshotVariants).filter(Boolean)).map(merchandiseProduct);
}
async function readProductSnapshot(env,id,{store=createSnapshotStore(env)}={}){
  ...
  const restricted=restrictSnapshotVariants(product);
  return restricted ? merchandiseProduct(catalogProducts([restricted])[0]) : null;
}
function restrictSnapshotVariants(product) {
  if(Number(product.id)!==473808622)return product;
  // Apply revocation on reads too, before the next complete provider refresh.
  const variants=product.variants.filter(v=>variantColorApproved(product,v));
  if(!variants.length)return null;
  const images=productImages[product.id];
  return {...product,variants,images,thumbnail_url:images[0],
