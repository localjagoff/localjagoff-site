  ...
  return {
    ...
    async product(id){
      const row=(await query(`SELECT (SELECT value FROM json_each(published)
        ...
        WHERE id=1 AND policy=?1 AND published IS NOT NULL
        AND published_source_at>strftime('%Y-%m-%dT%H:%M:%fZ','now','-150 minutes')
        AND published_source_at<=strftime('%Y-%m-%dT%H:%M:%fZ','now')`,[POLICY,id]))[0];
...
async function readProductSnapshot(env,id,{store=createSnapshotStore(env)}={}){
  ...
  return catalogProducts([product])[0];
}
...
module.exports={IDS,POLICY,MAX_AGE_MS,SCHEMA,configured,createSnapshotStore,readSnapshot,readProductSnapshot,refreshStep};
