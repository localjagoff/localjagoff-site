...
async function runCommunications({env=process.env,storeFactory=createStore,serviceFactory=createService,
  ...
  const service=serviceFactory({store,env,
  ...
  if(mode==='fast') {
    const sent=await dispatch(store,{env,beforeReview:service.beforeReview});
  ...
  try {
    const result=await service.reconcile(job.reference);
