...
const {hash,alertOwner} = require('./communications-store.cjs');
...
async function deliver(store, {key,env=process.env,send=sendViaResend,beforeReview,logger=console,now=Date.now()}={}) {
  ...
  if (job.kind==='processing') {
    await store.finish(job,'suppressed',{error:'processing_email_retired'});
    return {outcome:'processing_email_retired'};
  }
  ...
  if (job.kind==='review') {
    ...
    try {
      ...
      reviewCheck=check;
      if (!check.eligible) {
        const status=['waiting','event_during_review_check','fresh_provider_check_required'].includes(check.reason)?'pending':'suppressed';
        const due=Date.parse(check.dueAt);
        await store.finish(job,status,{error:check.reason,delay:Number.isFinite(due)?Math.max(1,Math.ceil((due-Date.now())/1000)):3600});
        return {outcome:status};
      }
    } catch {
      await alertOwner(store,job.order_ref,'fresh review provider verification failed');
      await store.finish(job,'pending',{error:'review_check_unavailable',delay:3600});
      return {outcome:'review_check_unavailable'};
    }
  }
  const attempt=await store.prepareAttempt(job,reviewCheck);
  if (attempt==='daily_mail_budget') {
    await store.finish(job,'pending',{error:'daily_mail_budget',delay:86400});
    return {outcome:'daily_mail_budget'};
  }
  let accepted;
  if(attempt==='review_changed_before_send'){
    await store.finish(job,'pending',{error:'review_changed_before_send',delay:3600});
    return {outcome:'review_changed_before_send'};
  }
  if(attempt!=='ready')throw new Error('outbox_attempt_not_persisted');
