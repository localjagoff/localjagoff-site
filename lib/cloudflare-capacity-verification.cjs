...
const SCHEMA='capacity_fixture_v7';
...
const CASES=['reconcile-small','reconcile-large','send-processing','fresh-review','retry-shipment','retry-not-due','terminal-not-due','cleanup'];
...
function providers({large=false,retry=false}={}){
  ...
  const calls={stripe:0,printful:0,email:0};
  ...
  const shipped_at=new Date(Date.now()-20*86400000).toISOString(),delivered_at=new Date(Date.now()-10*86400000).toISOString();
  ...
  const stripe=createLifecycleStripe(FIXTURE_ENV,{fetchImpl:async(url,init)=>{
      const u=new URL(url);
      if(u.origin!=='https://api.stripe.com'||u.pathname!==`/v1/checkout/sessions/${session.id}`||init.method!=='GET')throw Error('capacity_provider_path_denied');
      calls.stripe++;return Response.json(session);
    }});
  const fetchImpl=async(url,init)=>{
    const u=new URL(url),base=`/v2/orders/${id}`;
    if(u.origin!=='https://api.printful.com'||init.method!=='GET')throw Error('capacity_provider_path_denied');
    let data;
    if(u.pathname===base)data=order;
    else if([base+'/order-items',base+'/shipments'].includes(u.pathname)){
      const all=u.pathname.endsWith('order-items')?items:shipments;
      const offset=Number(u.searchParams.get('offset')||0);
      if(![0,100].includes(offset))throw Error('capacity_provider_path_denied');
      calls.printful++;
      return Response.json({data:all.slice(offset,offset+100),paging:{total:all.length},
        _links:offset+100<all.length?{next:{href:u.pathname+'?offset=100'}}:{}});
    }else throw Error('capacity_provider_path_denied');
    calls.printful++;return Response.json({data});
  };
