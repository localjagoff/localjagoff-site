...
async function request(req,env,{sqlFactory,storeFactory,fetchImpl=fetch,now=Date.now}={}){
  ...
  if(['capacity-seed','capacity-status','capacity-freeze'].includes(body)||/^capacity-r[0-7]$/.test(body)){
    if(!capacity.enabled(env,now()))return reply(404);
    if(/^capacity-r[0-7]$/.test(body))return reply(200,await capacity.run(env,{sqlFactory,phase:Number(body.slice(-1))}));
