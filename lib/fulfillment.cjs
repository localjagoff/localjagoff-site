...
async function ensureDraft(payload, session, options) {
  const path = `/orders/@${payload.external_id}?store_id=${STORE_ID}`;
  const check = (order) => {
    if (!order?.id || order.external_id !== payload.external_id || String(order.store) !== STORE_ID) {
      throw new CommerceError("Printful order identity mismatch", 503);
    }
    return order;
  };
  const existing = await printfulRequest(path, options);
  ...
  let order;
  try {
    order = check(await printfulRequest(`/orders?store_id=${STORE_ID}&confirm=false`, {
      ...options, method: "POST", body: payload,
    }));
  } catch (error) {
    // External IDs are store-unique: recover a concurrent create or a lost POST response.
    const recovered = await printfulRequest(path, options);
    if (recovered) return check(recovered);
    throw error;
  }
...
async function fulfillEvent(event, { stripe, fetchImpl = fetch, env = process.env, logger = console,
  ...
  // Printful has no payment sandbox. Test Stripe events must never reach the live store.
  ...
  try {
    ...
    const { items, lines } = await orderItems(session, stripe, options);
    ...
    const order = await ensureDraft({ external_id: orderId, recipient, retail_costs,
    ...
    logger.error("fulfillment_failed", { event_id: event.id, session_id: sessionId, external_id: orderId, code });
