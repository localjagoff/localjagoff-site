...
async function ensureDraft(payload, session, options, claimCreate) {
  ...
  if (claimCreate && !(await claimCreate(payload.external_id))) {
    const recovered = await findOrder(session, options);
    if (recovered) return { order: recovered, outcome: "create_attempt_recovered" };
    throw new CommerceError("Prior Printful create attempt requires manual recovery", 503);
  }
  let order;
  try {
    order = checkOrder(await printfulRequest(`/orders?store_id=${STORE_ID}&confirm=false&update_existing=false`, {
...
async function fulfillEvent(event, { stripe, fetchImpl = fetch, env = process.env, logger = console,
  sendEmail = async () => false, recordPaid, recordLinked, recordFailed, claimCreate } = {}) {
  ...
  if (event.livemode === false) {
    ...
    logger.info("fulfillment_test_skipped", {
      event_id: typeof event.id === "string" && /^evt_[A-Za-z0-9]{1,200}$/.test(event.id) ? event.id : null,
      session_id: typeof sessionId === "string" && /^cs_test_[A-Za-z0-9]{1,200}$/.test(sessionId) ? sessionId : null,
      outcome: "test_mode_no_printful_or_email",
    });
    return { received: true, skipped: "test_mode_no_printful" };
  }
  ...
  try {
    ...
    verifiedPaid = true;
    if (recordPaid) await recordPaid({session,event,reference:orderId});
    ...
    let lines;
    if (!order) {
      if (claimCreate && (session.metadata.printful_external_id === orderId ||
          session.metadata.fulfillment_state === "needs_retry_or_review")) {
        throw new CommerceError("Prior fulfillment attempt requires manual recovery", 503);
      }
      ...
      ({ order, outcome } = await ensureDraft({ external_id: orderId, recipient, retail_costs,
        items: items.map((i) => ({ sync_variant_id: i.variant_id, quantity: i.quantity,
          retail_price: money(i.unit_amount) })), confirm: false }, session, options, claimCreate));
    }
    linkedOrder = order;
    if (recordLinked) await recordLinked({session,reference:orderId,order});
    ...
    if (!recordPaid && session.metadata.order_email_sent !== "true") {
      try {
        recipient ||= recipientFrom(session);
    ...
    if (verifiedPaid && recordFailed) {
      try { await recordFailed({session,event,reference:orderId}); }
      catch { logger.error("owner_alert_persistence_failed", {session_id:sessionId,external_id:orderId}); }
    }
    ...
    if (session?.metadata?.store_id === STORE_ID) {
      try {
        await stripe.checkout.sessions.update(sessionId, { metadata: {
          ...
          ...(linkedOrder ? { printful_order_id: String(linkedOrder.id) } : {}),
