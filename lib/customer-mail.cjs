...
function orderConfirmation({ session, recipient, orderId, lineItems }) {
  ...
  return message(recipient.email || session.customer_details?.email || session.customer_email,
    ...
    [`Order ${orderId}`, "Payment received. Your order is confirmed and being prepared. We’ll send you another email as soon as your order ships with tracking information."],
...
function shipmentDate(value) {
  if (typeof value !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(value)) return "";
  const date = new Date(`${value}T00:00:00.000Z`);
  if (!Number.isFinite(date.getTime()) || date.toISOString().slice(0, 10) !== value) return "";
  // Preserve the provider's calendar date without a local-timezone shift.
  return `${value.slice(5, 7)}/${value.slice(8, 10)}/${value.slice(0, 4)}`;
}

function shipmentDateRange(eta) {
  const from = shipmentDate(eta?.from_date), to = shipmentDate(eta?.to_date);
...
function shipmentEmail(order, shipment) {
  ...
  const eta = shipmentDateRange(shipment.estimated_delivery);
  const facts = [shipment.is_reshipment ? "Replacement package" : "Package update", `Order: ${order.reference}`,
    `Carrier: ${shipment.carrier || "Carrier information is being updated"}`,
    `Tracking number: ${shipment.tracking_number || "Tracking is being updated"}`,
    eta ? `Estimated delivery: ${eta}. This is an estimate, not a delivery confirmation.` : "",
...
async function sendViaResend(payload, key, { env = process.env, fetchImpl = fetch } = {}) {
  ...
  if (!/^[a-zA-Z0-9/_-]{1,200}$/.test(key) || !validEmail(payload.reply_to) || !Array.isArray(payload.to) ||
    ...
    ![ORDER_SENDER, CONTACT_SENDER].includes(payload.from) || payload.bcc || payload.cc) throw new Error("invalid_email_envelope");
  const customerMail=/^(receipt|shipment|review)\//.test(key) && payload.from===ORDER_SENDER;
  const bcc=env.CUSTOMER_EMAIL_BCC||SUPPORT;
  if(customerMail&&!validEmail(bcc))throw new Error('invalid_customer_email_bcc');
  const envelope=customerMail&&payload.to[0].toLowerCase()!==bcc.toLowerCase()
    ? {...payload,bcc:[bcc]} : payload;
  let response;
  try {
    response = await fetchImpl("https://api.resend.com/emails", { method: "POST", redirect: "error",
      ...
      signal: AbortSignal.timeout(10000), body: JSON.stringify(envelope) });
...
module.exports = { SUPPORT, ORDER_SENDER, CONTACT_SENDER, escapeHtml, validEmail, safeUrl, money,
  orderConfirmation, shipmentEmail, reviewEmail, contactEmail, sendViaResend };
