...
// Keep signature-handler SDK initialization at startup after removing it from the scheduler.
require('stripe');
...
const {createReceiptHandler}=require('./checkout-receipt.cjs');
...
const catalogSnapshot=require('./catalog-snapshot.cjs');
...
function utf8Json(request){
  try{
    const type=contentType.parse(request.headers.get('content-type')||'');
    return ['application/json','application/ld+json'].includes(type.type)&&
      ['utf-8','utf8'].includes((type.parameters.charset||'utf-8').toLowerCase());
  }catch{return false;}
}
async function webJson(request,limit){
  // Bound actual bytes while retaining Next's UTF-8 replacement/BOM and empty-body behavior.
  const reader=request.body?.getReader(),decoder=new TextDecoder('utf-8');
  let body='',size=0;
  try{
    if(reader)for(;;){
      ...
      if(size>limit){
        ...
        throw Object.assign(Error('Payload too large'),{statusCode:413});
      }
    ...
    throw Object.assign(Error('Invalid body'),{statusCode:error.statusCode===413?413:400});
  }finally{reader?.releaseLock();}
}

function nodeRequest(request,stream=true){
  const req=stream?(request.body?Readable.fromWeb(request.body,{highWaterMark:0}):Readable.from([])):{destroy(){}};
...
function createApiAdapter({loadCatalog=catalog.loadCatalog,loadSnapshot=catalogSnapshot.readSnapshot,now=Date.now,
  webhookFactory=createWebhookHandler,printfulFactory=createPrintfulNotificationHandler,
  contactFactory=createContactHandler,reviewsFactory=createReviewsHandler,checkoutFactory=createCheckoutHandler,
  receiptFactory=createReceiptHandler}={}){
  const snapshots=new WeakMap();
  async function snapshot(env){
    const key=env.PRINTFUL_API_KEY,cached=snapshots.get(env);
    if(cached&&cached.key===key&&cached.mode===env.CATALOG_SNAPSHOT_ENABLED&&now()<cached.expires)return cached.products;
    // Share completed public data only. In-flight I/O must stay with its owning invocation.
    const products=env.CATALOG_SNAPSHOT_ENABLED==='true'?await loadSnapshot(env):await loadCatalog({apiKey:key});
    snapshots.set(env,{key,products,mode:env.CATALOG_SNAPSHOT_ENABLED,expires:now()+60000});return products;
  }
  return async function request(request,env){
    ...
    return budget.withBudget(async()=>{
      const kind=ROUTES[pathname],json=['contact','reviews','checkout','receipt'].includes(kind)&&utf8Json(request),
        req=nodeRequest(request,!json),res=nodeResponse(req.method);
      ...
      try{
        ...
        if(req.method==='OPTIONS'&&['google','openai','sitemap','indexnow'].includes(kind))res.status(400).end();
          ...
          req.body=json?await webJson(request,1024):await parseBody(req,'1kb');
          await receiptFactory({env})(req,res);
        }
        else if(['contact','reviews','checkout'].includes(kind)){
          req.body=json?await webJson(request,kind==='contact'?20480:kind==='checkout'?1048576:10240):
            await parseBody(req,kind==='contact'?'20kb':kind==='checkout'?'1mb':'10kb');
          await (kind==='contact'?contactFactory:kind==='checkout'?checkoutFactory:reviewsFactory)({env})(req,res);
