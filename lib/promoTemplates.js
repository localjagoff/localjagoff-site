...
function promoFirstComment(product, baseCta, tone) {
  if (tone === "clean") {
...
function promoLinkHelper(product) {
  return product?.id
    ? `Product link: ${promoProductUrl(product)}`
    : `Site link: ${promoProductUrl(product)}`;
}

function promoCtaBlock(baseCta, product, tone) {
  return [
    `Main CTA: ${baseCta}`,
    promoFirstComment(product, baseCta, tone),
    promoLinkHelper(product),
  ].join("\n");
}
...
export function makeFreePromoPack(product, options = {}) {
  ...
  const ctaBlock = promoCtaBlock(baseCta, cleanProduct, tone);
  ...
  return {
    ...
    warnings: [
      ...
      "CTA now includes first-comment and direct product-link helper text for faster posting.",
