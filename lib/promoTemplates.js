...
function promoNoteCaptionLine(notes, type, mode) {
  ...
  if (note.includes("less salesy") || note.includes("not salesy") || note.includes("less sale")) {
    return "No hard sell — just local gear with the right amount of attitude.";
  }
  if (note.includes("724")) {
    return type === "724 gear"
      ? "724 energy all the way."
      : "Western PA energy, without forcing it.";
  }
  if (note.includes("hoodie") || note.includes("cold") || note.includes("winter") || note.includes("fall")) {
    return "Built for hoodie-weather moods and gray-sky days.";
  }
  if (note.includes("gift") || note.includes("christmas") || note.includes("holiday")) {
    return "Gift it to the jagoff who already has enough opinions.";
  }
  if (note.includes("clean") || note.includes("ad safe") || mode === "clean_ad") {
    return "Clean, simple, and still local.";
  }
  if (note.includes("funny") || note.includes("joke")) {
    return "A quick local joke you can wear.";
  }
  if (note.includes("weekend") || note.includes("friday")) {
    return "Weekend energy, Western PA edition.";
  }

  return "";
}

function promoNoteContext(notes, type, mode) {
  const note = String(notes || "").trim();
  if (!note) return "";
  return `Extra notes applied: ${note}.`;
}
...
export function makeFreePromoPack(product, options = {}) {
  ...
  const internalContext = [promoNoteContext(options.notes, type, mode), promoToneContext(tone)]
    .filter(Boolean)
    .join(" ");
  ...
  return {
    ...
    tiktok_caption: `${tiktokOpen} ${hookThree}${publicInline} ${tiktokClose} ${formatPromoHashtags(
    ...
    clean_ad_version: `${adSafeLine}${publicBlock} ${cta}`,
