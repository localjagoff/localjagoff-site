...
export function makeFreePromoPack(product, options = {}) {
  ...
  const facebookOpen = pickPromoTemplate(PLATFORM_OPENERS.facebook, seed + 8);
  const instagramOpen = pickPromoTemplate(PLATFORM_OPENERS.instagram, seed + 9);
  const tiktokOpen = pickPromoTemplate(PLATFORM_OPENERS.tiktok, seed + 10);
  const shortsOpen = pickPromoTemplate(PLATFORM_OPENERS.youtube_shorts, seed + 11);
  ...
  const noteBlock = noteLine ? `\n\n${noteLine}` : "";
  const noteInline = noteLine ? ` ${noteLine}` : "";
  ...
  return {
    brand_angle: `${brandAngle}${noteLine ? ` ${noteLine}` : ""} No API call, no extra cost.`,
    facebook_post: `${facebookOpen}\n\n${hookOne}\n\n${edgeLine}${noteBlock}\n\n${facebookClose}`,
    instagram_caption: `${instagramOpen} ${hookTwo}${noteBlock}\n\n${instagramClose}\n\n${formatPromoHashtags(
      makePromoHashtags(cleanProduct)
    )}`,
    tiktok_caption: `${tiktokOpen} ${hookThree}${noteInline} ${tiktokClose} ${formatPromoHashtags(
    ...
    youtube_shorts_description: `${shortsOpen} ${name} from Local Jagoff. ${attitudeDescriptor}, no boring gear.${noteInline} ${shortsClose}`,
    ...
    short_video_script: [
      ...
      },
      {
        ...
        on_screen_text: name,
        voiceover: noteLine ? `${edgeLine} ${noteLine}` : edgeLine,
      },
      {
        scene: "Scene 3",
    ...
    clean_ad_version: `${adSafeLine}${noteBlock} ${cta}`,
    edgy_version: noteLine ? `${edgeLine} ${noteLine}` : edgeLine,
    cta,
    warnings: [
      "Free template mode did not use AI and costs $0.",
      noteLine ? "Extra notes were applied with simple non-AI keyword logic." : "Review before posting. No auto-publishing is enabled yet.",
    ],
  };
}
