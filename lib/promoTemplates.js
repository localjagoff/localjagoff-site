...
function promoFirstComment(name, baseCta, tone) {
  if (tone === "clean") {
...
function promoLinkHelper(name) {
  return `Link helper: Use localjagoff.com, or paste the product page link if you are posting a specific item.`;
}

function promoCtaBlock(baseCta, name, tone) {
  return [`Main CTA: ${baseCta}`, promoFirstComment(name, baseCta, tone), promoLinkHelper(name)].join("\n");
}
...
export function makeFreePromoPack(product, options = {}) {
  ...
  const ctaBlock = promoCtaBlock(baseCta, name, tone);
  ...
  return {
    ...
    warnings: [
      ...
      "CTA now includes first-comment and link-helper text for faster posting.",
