...
const PATHS=new Set(['/api/webhook','/api/printful-events','/api/create-checkout-session']);
  ...
  return {
    ...
    async fetch(request){
      ...
      return budget.withBudget(async()=>{
        ...
        if(new URL(request.url).pathname!=='/api/create-checkout-session'&&isProduction(env)&&env.COMMUNICATIONS_ENABLED==='true'&&request.method==='POST'&&response.ok){
