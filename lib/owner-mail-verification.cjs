...
function payload(mode = 'direct') {
  if (mode === 'scheduler') {
    const email = contactEmail({name:'Local Jagoff scheduler verification',email:SUPPORT,topic:'other',
      message:'OWNER-ONLY SCHEDULER TEST. The native Netlify timer dispatched an authenticated worker, which claimed this fixed durable job and sent it once. This is a separate scheduler check, not a repeat of the completed delivery test. No real order, payment, Printful access or customer email was created.'});
    return {...email,from:ORDER_SENDER,subject:'Local Jagoff - owner-only scheduled delivery test'};
  }
...
async function verify(request, {env = process.env, storeFactory = createStore, fetchImpl = fetch, now = Date.now(), mode = 'direct'} = {}) {
  const reply = (status, body) => Response.json(body, {status, headers: {'cache-control': 'no-store'}});
  const scheduler = mode === 'scheduler';
  const until = Date.parse(env.OWNER_SCHEDULER_VERIFICATION_UNTIL || '');
  if (scheduler && (!Number.isFinite(until) || until <= now || until > now + 3600000)) return reply(404, {outcome:'verification_window_closed'});
  const enabled = scheduler ? env.OWNER_SCHEDULER_VERIFICATION_ENABLED : env.OWNER_MAIL_VERIFICATION_ENABLED;
  if (enabled !== 'true' || env.COMMERCE_ENV !== 'preview' ||
  ...
  const store = storeFactory(env), key = `verification/owner-netlify-${scheduler ? 'scheduler-' : ''}v1/${env.SITE_ID}`, mail = payload(mode);
