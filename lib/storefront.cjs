const FEATURED_PRODUCT_IDS = [471744647, 471744585, 471950476, 471744283];
const SPOTLIGHT_PRODUCT_IDS = [473808622, 473834484];
const CURATED_PRODUCT_IDS = [...FEATURED_PRODUCT_IDS, 473689891, ...SPOTLIGHT_PRODUCT_IDS];
const CATEGORIES = [
  ...
  { key: '724', label: '724', href: '/724', description: 'Western PA. Same local attitude.' },
...
function inCategory(product, category) {
  ...
  if (category === '724') return [430964873, 473981186, 473991005, 473985115].includes(Number(product.id));
...
function sortCatalog(products, sort) {
  ...
  if (sort === 'curated') {
    const rank = id => {
      const index = CURATED_PRODUCT_IDS.indexOf(Number(id));
      return index < 0 ? CURATED_PRODUCT_IDS.length : index;
    };
    return [...products].sort((a, b) => rank(a.id) - rank(b.id));
  }
  return products;
}
module.exports = { FEATURED_PRODUCT_IDS, SPOTLIGHT_PRODUCT_IDS, CATEGORIES, SMALL_GOODS, money, displayName, inCategory, inSmallCategory, sortCatalog };
