...
function eligibleLocation(location) {
  ...
  if (!(/^\/(?:product\/[1-9]\d*|cart|success|privacy)?$/.test(url.pathname)) || url.hash) return false;
...
function createTracker(win) {
  let initialized = false, page = '', viewed = '', currentProduct = null;
  ...
  const allowed = () => choice() === 'granted' && eligibleLocation(win.location);
  ...
  function refresh() {
    if (!allowed()) { if (initialized) win.fbq('consent', 'revoke'); return; }
  ...
  return { choice, privacySignal, allowed, refresh, setChoice,
    suspend() { if (initialized) win.fbq('consent', 'revoke'); },
    observeProduct(item) {
