...
test('Production provisioning config remains dark and uses distinct storage and identity',()=>{
  ...
  assert.equal(production.workers_dev,false);assert.equal(production.preview_urls,false);assert.deepEqual(production.triggers.crons,[]);
  assert.equal(production.routes,undefined);assert.equal(production.vars.CHECKOUT_PAUSED,'true');
  assert.equal(production.vars.COMMUNICATIONS_ENABLED,'false');assert.equal(production.vars.CUSTOMER_EMAIL_ENABLED,'false');
