...
test('real SQL and lifecycle code use only isolated synthetic rows; providers never access network',async()=>{
  ...
  try{
    ...
    assert.deepEqual(results.map(r=>r.outcome),['reconciled','reconciled_partial','no_due_order','sent','retry_scheduled','no_due_order','no_due_order','cleanup_complete','paid_capacity_pass','paid_capacity_pass','paid_capacity_pass']);
