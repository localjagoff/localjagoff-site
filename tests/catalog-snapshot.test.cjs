...
test('provider failure preserves the previous full snapshot and does not advance',async()=>{
  ...
  const published=await snapshot.readSnapshot(env,{store});assert.equal(published.length,19);
  assert.ok(!published.some(p=>p.id===snapshot.IDS[0]));
});

test('Snapback replacement preserves the other 19 until a complete cycle, without extending freshness',async()=>{
  await reset();await cycle();
  ...
  assert.deepEqual(migrated.map(p=>p.variants),previous.filter(p=>p.id!==473808088).map(p=>p.variants));
  assert.equal(await snapshot.readProductSnapshot(env,473834484,{store}),null);
  ...
  assert.equal((await snapshot.readSnapshot(env,{store})).length,19);
...
test('compatibility never admits unknown products or a different policy',async()=>{
  await reset();await cycle();const before=await store.read();
  db.prepare('UPDATE public_catalog_snapshot SET policy=?,published=?').run(snapshot.PREVIOUS_POLICY,
    JSON.stringify([...before.published,product(999999)]));
  await assert.rejects(snapshot.readSnapshot(env,{store}),/policy_mismatch/);
  db.prepare('UPDATE public_catalog_snapshot SET policy=?').run(JSON.stringify([...snapshot.IDS,999999]));
  await assert.rejects(snapshot.readSnapshot(env,{store}),/unavailable/);
  await store.reset((await store.read()).version);
  assert.equal((await store.read()).published,null);
});
...
test('single-product D1 read preserves approval, omission and snapshot expiry without parsing the entire catalog',async()=>{
  ...
  assert.deepEqual(await snapshot.readProductSnapshot(env,id,{store}),require('../lib/product-merchandising.cjs').merchandiseProduct(product(id)));
