...
export default function PromoStudioCore() {
  ...
  const buildPartsFromPromo = (nextPromo, nextPlatform = platform) => {
    const nextParts = makeParts({ promo: nextPromo, product: selectedProduct, platform: nextPlatform, mode });
    setParts(nextParts);
    setSelected({
    ...
    <div className="promoPage">
      ...
      <main className="wrap">
        <header className="hero">
          <div><p className="kicker">PRIVATE ADMIN TOOL</p><h1>Promo Studio</h1><p>Facebook and Instagram promo builder for Local Jagoff posts. Generate, pick your parts, copy the final post, or add it to the queue.</p></div>
            ...
            {!promo || !parts ? <div className="empty"><h2>No promo generated yet.</h2><p>Generate a Facebook or Instagram pack first.</p><button type="button" className="primary" onClick={() => setActiveTab("create")}>Create One</button></div> : (
              <>
                ...
                <div className="builderGrid">
                  ...
                  <div className="parts">
                    <OptionGroup title="Opening Statement" field="opening" values={parts.opening} selected={selected.opening} onUse={onUsePart} />
